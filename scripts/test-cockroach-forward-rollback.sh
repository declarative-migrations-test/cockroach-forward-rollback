...
ADMIN="${COCKROACH_ADMIN_URL:-postgresql://root@localhost:26257/defaultdb?sslmode=disable}"
DB="dm_cockroach_forward_rollback"
TARGET="postgresql://root@localhost:26257/${DB}?sslmode=disable"
root="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
cleanup() { psql "$ADMIN" -v ON_ERROR_STOP=1 -c "DROP DATABASE IF EXISTS ${DB} CASCADE" >/dev/null 2>&1 || true; }
trap cleanup EXIT
cleanup
...
echo "CockroachDB forward/rollback certification passed"
