...
DB="dm_cockroachdb_rollback_e2e"
...
cleanup() {
  psql "$ADMIN" -v ON_ERROR_STOP=1 -c "DROP DATABASE IF EXISTS ${DB} CASCADE" >/dev/null 2>&1 || true
}
trap cleanup EXIT
cleanup
...
echo "CockroachDB rollback certification passed"
